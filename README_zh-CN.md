...
统计先验”。实验观察表明，embedding std 与 token 的候选可用性存在相关性：
低 std 区域集中了更多低可用性的投影候选。因此，PEZ-CFC 利用这种相关性缩小搜索
空间，把有限的优化步骤集中到更有效的词表区域，同时不需要引入额外语言模型。
...
的探测。两个端点组的代表样本展示了 std 与候选可用性的相关性：
...
`0.0281`。这项分桶观察提供了 std 与候选可用性相关的定性证据；后续的阈值扫描则
直接度量它作为候选筛选信号的实用价值。
...
- 优化目标和评估指标都来自 CLIP。CLIP 分数提升不等于 prompt 更自然，也不等于
  文生图质量一定提升。
- 实验已经观察到 std 与候选可用性的相关性；后续可以进一步量化这种相关性与 token
  频率、语义质量、自然度及训练覆盖程度之间的关系。
